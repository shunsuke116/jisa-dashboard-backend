...
def select_delivery_history(db: Session, limit: int = None) -> List[DeliveryHistoryDisplayed]:
...
def select_path_id_by_order_id(db: Session, order_id: str):
    ...
    for i in range(5):  # 最大5回繰り返す
        try:
            result = db.query(DeliveryHistory.path_id).filter(DeliveryHistory.order_id == order_id).first()
        except RuntimeError as e:
